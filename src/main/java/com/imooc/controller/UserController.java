...
public class UserController {
    ...
    public IMoocJSONResult registOrLogin(@RequestBody Users user) throws Exception {
        ...
        if (usernameIsExist) {
            ...
            user.setFaceImage("https://ss1.bdstatic.com/70cFuXSh_Q1YnxGkpoWK1HF6hhy/it/u=233321941,118003456&fm=26&gp=0.jpg");
            user.setFaceImageBig("https://ss1.bdstatic.com/70cFuXSh_Q1YnxGkpoWK1HF6hhy/it/u=233321941,118003456&fm=26&gp=0.jpg");
    ...
    public IMoocJSONResult uploadFace(@RequestParam("uploadFile") MultipartFile uploadFile, HttpServletRequest request) throws IOException {
        System.out.println("abc访问ssd122到的图e片上传接口");
        if (null == uploadFile) {
            return IMoocJSONResult.errorMsg("1上传失败，无法找到文件！");
        }
        ...
        if (!fileName.endsWith(".bmp") && !fileName.endsWith(".jpg")
            ...
            return IMoocJSONResult.errorMsg("上传失败，请选择BMP、JPG、JPEG、PNG、GIF文件！");
        }
        System.out.println("文件名字为：" + uploadFile.getOriginalFilename());//打印文件名字
        // 上传文件到fastdfs
        //MultipartFile faceFile = FileUtils.fileToMultipart(userFacePath);
        MultipartFile faceFile = uploadFile;
        String url = fastDFSClient.uploadBase64(faceFile);
        System.out.println(url);

        UsersBO userBO = new UsersBO();
        userBO.setUserId(request.getParameter("userId"));
...
//		"dhawuidhwaiuh3u89u98432_80x80.png"
    ...
    public IMoocJSONResult operFriendRequest(String acceptUserId, String sendUserId,
        ...
        // 4. 数据库查询好友列表
        Map<String,List<MyFriendsVO>> myFirends = userService.queryMyFriends(acceptUserId);
    ...
    public IMoocJSONResult myFriends(String userId) {
        ...
        // 1. 数据库查询好友列表
        Map<String,List<MyFriendsVO>> myFirends = userService.queryMyFriends("1904249A61MSZ7C0");
        System.out.println(myFirends);
        Map<String,Object> map = new HashMap<>();
        map.put("myFriends",myFirends);
